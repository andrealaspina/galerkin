clearvars -except Files Passed CPUTime Test;

%% Input

% Simulation ---------------------------------------------------------------------------------------
Simulation.Type='SingleSimulation';              % Simulation type
Simulation.Problem='FluidStructureInteraction';  % Problem
% --------------------------------------------------------------------------------------------------

% Parameters ---------------------------------------------------------------------------------------
Parameters(1).Formulation='WeaklyCompressibleFlowVP_HDG';% Formulation
Parameters(1).Problem='Fluid';                   % Problem
Parameters(1).PostProcessingHDG='no';            % Perform HDG postprocessing
Parameters(1).ConvectiveFlow='yes';              % Convective flow
Parameters(1).ArbitraryLagrangianEulerian='yes'; % Arbitrary Lagrangian-Eulerian description
Parameters(1).Degree=1;                          % Degree
Parameters(1).StabVelocity=1;                    % Stabilization for velocity
Parameters(1).StabPressure=1;                    % Stabilization for pressure
Parameters(1).ReferenceDensity=1;                % Reference density
Parameters(1).ReferencePressure=0;               % Reference pressure
Parameters(1).CompressibilityCoeff=0.001;        % Compressibility coefficient
Parameters(1).DynamicViscosity=0.01;             % Dynamic viscosity
Parameters(1).ScaledStrainRate=...               % Scaled strain rate
  @(x,y,z,t) [0*x,0*x,0*x,0*x,0*x,0*x];
Parameters(1).Velocity=...                       % Velocity
  @(x,y,z,t) [(abs(x)<1e-3).*(1-4*y.^2).*(1-16*z.^2)*0.1*((1-cos(pi*t/5))/2*(t<=5)+(t>5)),...
              0*x,...
              0*x];
Parameters(1).Pressure=@(x,y,z,t) 0*x;           % Pressure
Parameters(1).Traction=@(x,y,z,t) [0*x,0*x,0*x]; % Traction
Parameters(1).ResidualContinuity=@(x,y,z,t) 0*x; % Residual of continuity equation
Parameters(1).Force=@(x,y,z,t) [0*x,0*x,0*x];    % Force
Parameters(1).Density=@(Pressure)...             % Equation of state
  Parameters(1).ReferenceDensity+...
 (Pressure-Parameters(1).ReferencePressure)*Parameters(1).CompressibilityCoeff;
Parameters(1).DDensityDPressure=@(Pressure)...   % dDensity/dPressure
  Parameters(1).CompressibilityCoeff;
Parameters(1).Displacement=...                   % Displacement
  @(x,y,z,t) [0*x,0*x,0*x];
Parameters(2).Formulation='Elasticity_CG';       % Formulation
Parameters(2).Problem='Mesh';                    % Problem
Parameters(2).Model='LinearElasticity';          % Model
Parameters(2).Degree=1;                          % Degree
Parameters(2).NitschePenalty=1e6;                % Nitsche's penalty parameter
Parameters(2).Density=0;                         % Density
Parameters(2).YoungsModulus=1;                   % Young's modulus
Parameters(2).PoissonsRatio=0;                   % Poisson's ratio
Parameters(2).Displacement=...                   % Displacement
  @(x,y,z,t) [0*x,0*x,0*x];
Parameters(2).Traction=...                       % Traction
  @(x,y,z,t,b,nx,ny,nz) [0*x,0*x,0*x];
Parameters(2).Force=@(x,y,z,t) [0*x,0*x,0*x];    % Force
Parameters(3).Formulation='Elasticity_CG';       % Formulation
Parameters(3).Problem='Structural';              % Problem
Parameters(3).Model='NeoHook';                   % Model
Parameters(3).Degree=1;                          % Degree
Parameters(3).NitschePenalty=1e6;                % Nitsche's penalty parameter
Parameters(3).Density=1;                         % Density
Parameters(3).YoungsModulus=500;                 % Young's modulus
Parameters(3).PoissonsRatio=0;                   % Poisson's ratio
Parameters(3).Displacement=...                   % Displacement
  @(x,y,z,t) [0*x,0*x,0*x];
Parameters(3).Traction=...                       % Traction
  @(x,y,z,t,b,nx,ny,nz) [0*x,0*x,0*x];
Parameters(3).Force=@(x,y,z,t) [0*x,0*x,0*x];    % Force
% --------------------------------------------------------------------------------------------------

% Geometry and mesh --------------------------------------------------------------------------------
Mesh.File={'Mesh_fsi_3d_channel'};               % Mesh file
% --------------------------------------------------------------------------------------------------

% System -------------------------------------------------------------------------------------------
System.SymmetrizeMatrix='no';                    % Symmetrize matrix
System.Nonlinear='yes';                          % Nonlinear
System.Tolerance=1e-6;                           % Tolerance
System.MaxIterations=20;                         % Maximum number of iterations
% --------------------------------------------------------------------------------------------------

% Time parameters ----------------------------------------------------------------------------------
Time.TimeDependent='yes';                        % Time dependent problem
Time.InitialTime=0;                              % Initial time
Time.FinalTime=0.3;                              % Final time
Time.TimeStepSize=0.1;                           % Time step size
Time.BDFOrder=2;                                 % BDF order
% --------------------------------------------------------------------------------------------------

% Solver -------------------------------------------------------------------------------------------
Solver.Type='backslash';                         % Type
% --------------------------------------------------------------------------------------------------

% Boundary splitting -------------------------------------------------------------------------------
Boundaries(1).Dirichlet_v_x=[2,3,4,5,7];         % Dirichlet portion for x-velocity
Boundaries(1).Dirichlet_v_y=[2,3,4,5,7];         % Dirichlet portion for y-velocity
Boundaries(1).Dirichlet_v_z=[2,3,4,5,7];         % Dirichlet portion for z-velocity
Boundaries(1).Dirichlet_p=8;                     % Dirichlet portion for pressure
Boundaries(1).Interface=[1,6,9,10,11];           % Interface portion
Boundaries(1).Neumann_t_x=[];                    % Neumann portion for x-traction
Boundaries(1).Neumann_t_y=[];                    % Neumann portion for y-traction
Boundaries(1).Neumann_t_z=[];                    % Neumann portion for z-traction
Boundaries(2).Dirichlet=[2,3,4,5,7,8];           % Dirichlet portion
Boundaries(2).Interface=[1,6,9,10,11];           % Interface portion
Boundaries(2).Neumann=[];                        % Neumann portion
Boundaries(3).Dirichlet=[];                      % Dirichlet portion
Boundaries(3).Interface=[1,2,3,4,6];             % Interface portion
Boundaries(3).Neumann=[];                        % Neumann portion
Boundaries(3).Fixed=5;                           % Fixed portion
% --------------------------------------------------------------------------------------------------

% Output options -----------------------------------------------------------------------------------
Options.PlotGeometry='no';                       % Plot geometry
Options.PlotMesh='no';                           % Plot mesh
Options.ComputeError=...                         % Compute error
  {'ScaledStrainRate';
   'Velocity';
   'Pressure';
   'Displacement'};
Options.Test=...                                 % Test
  ['abs(Results(1).ScaledStrainRateErrorL2-4.148424739576812e-04)<1e-12 && ',...
   'abs(Results(1).VelocityErrorL2        -4.205155756224068e-04)<1e-12 && ',...
   'abs(Results(1).PressureErrorL2        -7.483614587381608e-03)<1e-08 && ',...
   'abs(Results(1).DisplacementErrorL2    -8.908720254695782e-06)<1e-12 && ',...
   'abs(Results(2).DisplacementErrorL2    -8.908720254695782e-06)<1e-12 && ',...
   'abs(Results(3).DisplacementErrorL2    -2.434608304510903e-06)<1e-12'];
% --------------------------------------------------------------------------------------------------

%% Main

main