...
Parameters(2).NodesDistribution='Fekete';        % Nodes distribution
...
Options.ComputeError=...                         % Compute error
  {'ScaledTemperatureGradient';
   ...
   'TemperaturePost'};
Options.Test=...                                 % Test
  ['max(Results(1).ScaledTemperatureGradientErrorL2)<1e-10 && ',...
   'max(Results(1).TemperatureErrorL2              )<1e-11 && ',...
   'max(Results(1).TemperaturePostErrorL2          )<1e-11 && ',...
   'max(Results(2).TemperatureErrorL2              )<1e-11'];
% --------------------------------------------------------------------------------------------------
