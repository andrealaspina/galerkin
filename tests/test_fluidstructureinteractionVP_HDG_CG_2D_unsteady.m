...
Parameters(1).StabVelocity=1000;                 % Stabilization for velocity
Parameters(1).StabPressure=1000;                 % Stabilization for pressure
...
Parameters(1).Velocity=...                       % Velocity
  @(x,y,z,t) [(abs(x)<1e-3)*3/2.*y.*(0.41-y)/(0.41/2)^2*1*((1-cos(pi*t/2))/2*(t<=2)+(t>2)),...
              0*x];
...
Options.Test=...                                 % Test
  ['abs(Results(1).ScaledStrainRateErrorL2-1.299915055226722e-02)<1e-12 && ',...
   'abs(Results(1).VelocityErrorL2        -5.057721586910789e-04)<1e-12 && ',...
   'abs(Results(1).PressureErrorL2        -5.528382011956940e+01)<1e-08 && ',...
   'abs(Results(1).DisplacementErrorL2    -2.766302313141373e-07)<1e-12 && ',...
   'abs(Results(2).DisplacementErrorL2    -2.766329850201201e-07)<1e-12 && ',...
   'abs(Results(3).DisplacementErrorL2    -6.010388658540998e-08)<1e-12 && ',...
   'abs(Results(3).DisplacementTip        +2.212393593071357e-08)<1e-12'];
% --------------------------------------------------------------------------------------------------
