...
Simulation.ParameterStudy='CompressibilityCoeff';% Parameter for parametric study
% --------------------------------------------------------------------------------------------------
...
Parameters.CompressibilityCoeff=[1,0.1,0]/30;    % Compressibility coefficient
...
eps=Parameters.CompressibilityCoeff(1); mu=Parameters.DynamicViscosity;
...
Parameters.Density=@(Pressure)...                % Equation of state
 ...
 (Pressure-Parameters.ReferencePressure)*Parameters.CompressibilityCoeff(1);
Parameters.DDensityDPressure=@(Pressure)...      % dDensity/dPressure
  Parameters.CompressibilityCoeff(1);
...
Options.ComputeQuantityEnd=...                   % Compute quantity of interest (end of simulation)
  ['eps=Input.Parameters(iD).(Simulation.ParameterStudy)(min(iS2+1,Simulation.NumSimulations));',...
   'r0=Parameters.ReferenceDensity;         p0=Parameters.ReferencePressure;',...
   ...
   'p= @(x,y) p0+3*(mu*L*U)/(R^2)*(1-x/L)',...
   '          -3/2*(mu^2*U^2)/(r0*R^2)*(3*(L/R)^2*(1-x/L).^2-(1-(y/R).^2))*eps;',...
   'Parameters.ScaledStrainRate=',...
   '  @(x,y,z,t) [-(3*U^2*eps*mu^(3/2)*(R^2-y.^2)*(3*2^(1/2)+6^(1/2)))/(4*R^4*r0),',...
   '              +(3*U^2*eps*mu^(3/2)*(R^2-y.^2)*(3*2^(1/2)-6^(1/2)))/(4*R^4*r0),',...
   '              +(3*U*mu^(1/2)*y.*(R^2*r0-3*L*U*eps*mu+3*U*eps*mu*x))/(R^4*r0)];',...
   'Parameters.Velocity=@(x,y,z,t) [vx(x,y),vy(x,y)];',...
   'Parameters.Pressure=@(x,y,z,t) p(x,y);',...
   'Parameters.Traction=',...
   '  @(x,y,z,t) [-((x<(0+1e-6)) |(x>(L-1e-6))) .*sign(x-L/2).*',...
   '                (+p0-(3*L*U*mu*(x/L-1))/R^2-(3*U^2*eps*mu^2*(y.^2/R^2+(3*L^2*(x/L-1).^2)/R^2-1))/',...
   '                (2*R^2)+(6*U^2*eps*mu^2*(y.^2/R^2-1))/R^2)-',...
   ...
   '                (-p0+(3*L*U*mu*(x/L-1))/R^2+(3*U^2*eps*mu^2*(y.^2/R^2+(3*L^2*(x/L-1).^2)/R^2-1))/',...
   '                (2*R^2)+(3*U^2*eps*mu^2*(y.^2/R^2-1))/R^2)];',...
   'Parameters.ResidualContinuity=',...
   '  @(x,y,z,t) (27*mu^2*U^3)/(4*r0*R^8)*(R^2-y.^2).*',...
   '             (6*R^2*(L-x)-mu*U/r0*(9*(L-x).^2-R^2+y.^2)*eps)*eps^2;',...
   'Parameters.Force=@(x,y,z,t) [0*x,0*x];',...
   'Parameters.Density=@(Pressure)',...
   '  Parameters.ReferenceDensity+',...
   ' (Pressure-Parameters.ReferencePressure)*eps;',...
   'Parameters.DDensityDPressure=@(Pressure)',...
   '  eps;'];
Options.Test=...                                 % Test
  ['Results.ScaledStrainRateErrorL2(end)<1e-11 && ',...
   'Results.VelocityErrorL2(end)        <1e-11 && ',...
   'Results.PressureErrorL2(end)        <1e-10 && ',...
   'Results.VelocityPostErrorL2(end)    <1e-11'];
% --------------------------------------------------------------------------------------------------
