...
Parameters.VelocityCenter=...                    % Velocity at the center
  @(t) Parameters.Velocity(L/2,0,0,t);
...
Options.ComputeQuantityError=...                 % Compute quantity of interest (error computation)
  ['[~,Node]=ismember([5,0,0],Mesh.Nodes'',''rows''); ',...
   'Results.VelocityCenter=Results.Velocity(Node,:);'];
Options.Test=...                                 % Test
  ['Results.VelocityErrorL2          <1e-11 && ',...
   'Results.VelocityErrorHdiv        <1e-11 && ',...
   'Results.VelocityErrorHcurl       <1e-11 && ',...
   'Results.VelocityCenterErrorNumber<1e-11'];
% --------------------------------------------------------------------------------------------------
