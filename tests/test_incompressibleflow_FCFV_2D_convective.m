...
Parameters.ScaledStrainRate=...                  % Scaled strain rate
  @(x,y,z,t) [+(4*r1^2*r2^2*x.*y*(w1-w2))./((x.^2+y.^2).^2*(r1^2-r2^2)),...
              -(4*r1^2*r2^2*x.*y*(w1-w2))./((x.^2+y.^2).^2*(r1^2-r2^2)),...
              -(2*r1^2*r2^2*(x.^2-y.^2)*(w1-w2))./((x.^2+y.^2).^2*(r1^2-r2^2))];
...
Options.ComputeError=...                         % Compute error
  {'ScaledStrainRate';
   ...
   'Pressure'};
Options.Test=...                                 % Test
  ['abs(Results.ScaledStrainRateErrorL2-1.101055352576983e-00)<1e-12 && ',...
   'abs(Results.VelocityErrorL2        -2.184616444634213e-01)<1e-12 && ',...
