...
% Boundary splitting -------------------------------------------------------------------------------
Boundaries(1).Dirichlet=[1,4];                   % Dirichlet portion
...
% Regions of interest ------------------------------------------------------------------------------
Regions(1).Middle.Definition=...                 % Middle region (definition)
  @(x,y,z) (x>=-1/2).*(x<=+1/2).*(y==0);
Regions(1).Middle.Condition=@(x,y,z) y<0;        % Middle region (condition)
Regions(2).Middle.Definition=...                 % Middle region (definition)
  @(x,y,z) (x>=-1/2).*(x<=+1/2).*(y==0);
Regions(2).Middle.Condition=@(x,y,z) y<0;        % Middle region (condition)
% --------------------------------------------------------------------------------------------------
...
Options.ComputeError=...                         % Compute error
  {'ScaledTemperatureGradient';
   ...
   'TemperaturePost'};
Options.Test=...                                 % Test
  ['min(min(Faces(1,1).Middle==[8,3])) && ',...
   'min(min(Faces(2,2).Middle==[4,2]))'];
% --------------------------------------------------------------------------------------------------
