...
Parameters(1).ScaledStrainRate=...               % Scaled strain rate
  @(x,y,z,t) [-2^(1/2)*mu^(1/2)*pi*cos(pi*x).*sin(pi*y)*sin(pi*t),...
              +2^(1/2)*mu^(1/2)*pi*cos(pi*x).*sin(pi*y)*sin(pi*t),...
              0*x];
...
Options.Test=...                                 % Test
  ['abs(Results(1).ScaledStrainRateCenterErrorNumber-6.257078621031293e-01)<1e-12 && ',...
   ...
   'abs(Results(2).DisplacementCenterErrorNumber    -3.659813721148478e-17)<1e-12'];
% --------------------------------------------------------------------------------------------------
