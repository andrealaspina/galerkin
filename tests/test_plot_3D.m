...
Parameters(1).ThermalFlux=@(x,y,z,t) -1*(x==x);  % Thermal flux
...
Parameters(2).ThermalFlux=@(x,y,z,t) +1*(x==x);  % Thermal flux
Parameters(2).HeatSource=@(x,y,z,t) 0*x;         % Heat source
% --------------------------------------------------------------------------------------------------

% Geometry and mesh --------------------------------------------------------------------------------
Mesh.File={'Mesh_cube_coupled_1'};               % Mesh file
% --------------------------------------------------------------------------------------------------
...
% Boundary splitting -------------------------------------------------------------------------------
Boundaries(1).Dirichlet=[2,3,5,6];               % Dirichlet portion
Boundaries(1).Interface=1;                       % Interface portion
Boundaries(1).Neumann=4;                         % Neumann portion
Boundaries(2).Dirichlet=[2,3,5,6];               % Dirichlet portion
Boundaries(2).Interface=4;                       % Interface portion
Boundaries(2).Neumann=1;                         % Neumann portion
% --------------------------------------------------------------------------------------------------
