...
Options.ComputeQuantityTimeStep=...              % Compute quantity of interest at each time step
  ['Mesh.Nodes=Mesh.NodesInitial+',...
   '  [Parameters.Displacement(Mesh.NodesInitial(1,:)'',',...
   '                           Mesh.NodesInitial(2,:)'',',...
   '                           Mesh.NodesInitial(3,:)'',',...
   '                           Time.Time)'';',...
   '   zeros(1,size(Mesh.NodesInitial,2))];',...
   'Elements.Nodes=mat2cell(Mesh.Nodes(:,Mesh.Elements(:)),',...
   '3,ones(Sizes.NumElements,1)*Sizes.NumElementNodes)'';'];
Options.ComputeError=...                         % Compute error
  {'ScaledStrainRate';
   'Velocity';
   'Pressure'};
Options.Test=...                                 % Test
  ['abs(Results.ScaledStrainRateErrorL2-4.147181828332527e-01)<1e-12 && ',...
   'abs(Results.VelocityErrorL2        -1.348961628520154e-01)<1e-12 && ',...
   'abs(Results.PressureErrorL2        -2.026620566877048e-01)<1e-12'];
% --------------------------------------------------------------------------------------------------
