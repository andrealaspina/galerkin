function [Ionization]=solveSahaEquationFast(...
         Parameters,IndependentVariable,DensityOrPressure,Temperature)
         % Solve Saha equation
...
if strcmp(IndependentVariable,'Density')
  rho=DensityOrPressure;
elseif strcmp(IndependentVariable,'Pressure')
  P=DensityOrPressure;
end
T=Temperature;
...
if strcmp(Parameters.PartitionFunctionsExcitedStates,'yes')
  ...
  for j=1:J
    for r=1:Z(j)
      ...
      U{j}(r)=sum(g{j}{r}(1:nstar).*exp(-E{j}{r}(1:nstar)/(kb*T)));
    end
    U{j}=[U{j},1]; % not sure about this
  end
end
...
for j=1:J
  for r=1:Z(j)
    f{j,r+1}=@(Zav) 2*U{j}(r+1)/U{j}(r)*(2*pi*me*kb*T/h^2)^(3/2)*exp(-(I{j}(r)-DI{r}(Zav))/(kb*T));
    f0=f{j,r+1}(0);
    if f0>f0max
      f0max=f0;
    end
...
if strcmp(Parameters.SahaNonIdeal,'yes') && ...
  ...
  while err>tolN && it<itmax
    ...
    for j=1:J
      for r=1:Z(j)
        eta(j,r+1)=2*U{j}(r+1)/U{j}(r)*(2*pi*me/h^2)^(3/2)*(kb*T)^(5/2)*exp(-(I{j}(r)-DI(r))/(kb*T));
      end
    end
    etamax=max(max(eta));
