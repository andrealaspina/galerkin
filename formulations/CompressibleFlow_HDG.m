classdef CompressibleFlow_HDG < Formulation  
  ...
  methods
    ...
    function [PointData,CellData]=dataForParaview(~,Results,Parameters,~,Sizes,isPostProcess)
      ...
      % Write specific heat at constant pressure
      cp=Parameters.SpecificHeatConstantPressure;
      cp_DG=zeros(Sizes.NumElements,1);
      for iElem=1:Sizes.NumElements
        cp_DG(iElem,:)=cp;
      end
      CellData=[sprintf('\nSCALARS Specific_heat_constant_pressure float\n'),...
                sprintf('LOOKUP_TABLE default\n'),...
                sprintf('%.12f\n',cp_DG')];
      
      % Write specific heat at constant volume
      cv=Parameters.SpecificHeatConstantVolume;
      cv_DG=zeros(Sizes.NumElements,1);
      for iElem=1:Sizes.NumElements
        cv_DG(iElem,:)=cv;
      end
      CellData=[CellData,sprintf('\nSCALARS Specific_heat_constant_volume float\n'),...
                sprintf('LOOKUP_TABLE default\n'),...
                sprintf('%.12f\n',cv_DG')];
