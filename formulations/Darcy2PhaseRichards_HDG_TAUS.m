classdef Darcy2PhaseRichards_HDG_TAUS < Formulation  
...
tauSref=Parameters.StabSaturation;
t=Time.Time;
if isTimeDependent
...
for iFace=1:NumElementFaces
  ...
  if ComputeFace
    ...
    Kss(nf1,nf1)=Kss(nf1,nf1)+NwfT*((max(tauSref*dGdSfg,tauSref)).*Nf);

    KsS(nf1,nefS1)=KsS(nf1,nefS1)-NwfT*((max(tauSref*dGdSfg,tauSref)).*Nf);
    ...
    if isInterior || isNeumann_s
      ...
      KSs(nefS1,nf1)=KSs(nefS1,nf1)-NwfT*((max(tauSref*dGdSfg,tauSref)).*Nf);

      KSS(nefS1,nefS1)=KSS(nefS1,nefS1)+NwfT*((max(tauSref*dGdSfg,tauSref)).*Nf);
    ...
    fs(nf1,1)=fs(nf1,1)-NwfT*(V2nfg+max(tauSref*dGdSfg,tauSref).*(sfg-Sfg));
    ...
    if isInterior
      fS(nefS1,1)=+NwfT*(V2nfg+max(tauSref*dGdSfg,tauSref).*(sfg-Sfg));
    elseif isNeumann_s
      fS(nefS1,1)=+NwfT*(V2nfg+max(tauSref*dGdSfg,tauSref).*(sfg-Sfg)+f2Nfg/r2);
    elseif isDirichlet_s
      fS(nefS1,1)=-NwfT*(Sfg-sDfg);
    elseif isDoNothing_s
      fS(nefS1,1)=+NwfT*(max(tauSref*dGdSfg,tauSref).*(sfg-Sfg));
    end
  end
end
