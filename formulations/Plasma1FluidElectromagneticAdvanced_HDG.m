classdef Plasma1FluidElectromagneticAdvanced_HDG < Formulation  
  ...
  methods
    ...
    function [Block]=computeInitialConditions(~,iD,Block,Parameters,Mesh,Faces,Time,RefElement,...
      ...
      if strcmp(Time.TimeDependent,'yes')
        if not(matchField(Time,'SteadySolution','yes'))
          ...
          % Impose initial fields from initialization step
          if matchField(Parameters(iD),'ElectrostaticInitialization')
            [ResultsInitialization]=Parameters(iD).ElectrostaticInitialization(Parameters(iD));
            r=Block(iD,iD).SolutionLocal(:,1);
            B=Parameters(iD).MagneticPermeability*Block(iD,iD).SolutionLocal(:,...
              1+Sizes(iD).NumSpaceDim+1+(1:Sizes(iD).NumSpaceDim*(Sizes(iD).NumSpaceDim-1)/2));
            E=ResultsInitialization.ElectricFieldInitial;
            if Sizes(iD).NumSpaceDim==2
              v=[+E(:,2).*B,...
                 -E(:,1).*B]./vecnorm(B,2,2).^2;
            elseif Sizes(iD).NumSpaceDim==3
              v=[E(:,2).*B(:,3)-E(:,3).*B(:,2),...
                 E(:,3).*B(:,1)-E(:,1).*B(:,3),...
                 E(:,1).*B(:,2)-E(:,2).*B(:,1)]./vecnorm(B,2,2).^2;
            end
            % Momentum
            Block(iD,iD).SolutionLocal(:,1+(1:Sizes(iD).NumSpaceDim))=r.*v;
            % Energy
            Block(iD,iD).SolutionLocal(:,1+Sizes(iD).NumSpaceDim+1)=0;
            % ElectricField
            Block(iD,iD).SolutionLocal(:,1+Sizes(iD).NumSpaceDim+1+...
              Sizes(iD).NumSpaceDim*(Sizes(iD).NumSpaceDim-1)/2+(1:Sizes(iD).NumSpaceDim))=E;
          end
    ...
    function [PointData,CellData]=dataForParaview(~,Results,Parameters,Mesh,Sizes,isPostProcess)
      ...
      if matchField(Results,'Time')
        ...
        if isa(Parameters.MagneticPermeability,'function_handle')
          mu=Parameters.MagneticPermeability(Mesh.Nodes(1,:)',Mesh.Nodes(2,:)',Mesh.Nodes(3,:)');
        else
          mu=Parameters.MagneticPermeability*ones(size(Mesh.Nodes,2),1);
        end
        ...
        if isa(Parameters.ElectricPermittivity,'function_handle')
          epsilon=Parameters.ElectricPermittivity(Mesh.Nodes(1,:)',Mesh.Nodes(2,:)',Mesh.Nodes(3,:)');
        else
          epsilon=Parameters.ElectricPermittivity*ones(size(Mesh.Nodes,2),1);
        end
        ...
        if isa(Parameters.ElectricConductivity,'function_handle')
          sigma=Parameters.ElectricConductivity(Mesh.Nodes(1,:)',Mesh.Nodes(2,:)',Mesh.Nodes(3,:)');
        else
          sigma=Parameters.ElectricConductivity*ones(size(Mesh.Nodes,2),1);
        end
        ...
        gamma_DG=zeros(Sizes.NumElements,1);
        for iElem=1:Sizes.NumElements
          gamma_DG(iElem,:)=gamma;
        end
        CellData=[sprintf('\nSCALARS Specific_heat_ratio float\n'),...
                  sprintf('LOOKUP_TABLE default\n'),...
                  sprintf('%.12f\n',gamma_DG')];
...
for iFace=1:NumElementFaces
  ...
  if ComputeFace
    ...
    % Gram-Schmidt orthonormalization (straight faces in 3D)
    if nsd==3
      V=Xf(nefR,1:nsd)';
    ...
    if nsd==2
      KHE(nf1,nefE1)=KHE(nf1,nefE1)+NwfT*((Rxt1.*ny-Ryt1.*nx).*Nf);
    else
      KHE(nf1,nefE1)=KHE(nf1,nefE1)+NwfT*((Ryt1.*nz-Rzt1.*ny).*Nf);
      KHE(nf2,nefE1)=KHE(nf2,nefE1)+NwfT*((Rzt1.*nx-Rxt1.*nz).*Nf);
      KHE(nf3,nefE1)=KHE(nf3,nefE1)+NwfT*((Rxt1.*ny-Ryt1.*nx).*Nf);
      KHE(nf1,nefE2)=KHE(nf1,nefE2)+NwfT*((Ryt2.*nz-Rzt2.*ny).*Nf);
      KHE(nf2,nefE2)=KHE(nf2,nefE2)+NwfT*((Rzt2.*nx-Rxt2.*nz).*Nf);
      KHE(nf3,nefE2)=KHE(nf3,nefE2)+NwfT*((Rxt2.*ny-Ryt2.*nx).*Nf);
    end
    ...
    if nsd==2
      KeE(nf1,nefE1)=KeE(nf1,nefE1)-NwfT*((tauE*Rxt1).*Nf);
      KeE(nf2,nefE1)=KeE(nf2,nefE1)-NwfT*((tauE*Ryt1).*Nf);
    else
      KeE(nf1,nefE1)=KeE(nf1,nefE1)-NwfT*((tauE*Rxt1).*Nf);
      KeE(nf2,nefE1)=KeE(nf2,nefE1)-NwfT*((tauE*Ryt1).*Nf);
      KeE(nf3,nefE1)=KeE(nf3,nefE1)-NwfT*((tauE*Rzt1).*Nf);
      KeE(nf1,nefE2)=KeE(nf1,nefE2)-NwfT*((tauE*Rxt2).*Nf);
      KeE(nf2,nefE2)=KeE(nf2,nefE2)-NwfT*((tauE*Ryt2).*Nf);
      KeE(nf3,nefE2)=KeE(nf3,nefE2)-NwfT*((tauE*Rzt2).*Nf);
    end
    ...
    if isInterior
      if nsd==2
        KEH(nefE1,nf1)=KEH(nefE1,nf1)+NwfT*((Rxt1.*ny-Ryt1.*nx).*Nf);
      else
        KEH(nefE1,nf1)=KEH(nefE1,nf1)+NwfT*((Ryt1.*nz-Rzt1.*ny).*Nf);
        KEH(nefE2,nf1)=KEH(nefE2,nf1)+NwfT*((Ryt2.*nz-Rzt2.*ny).*Nf);
        KEH(nefE1,nf2)=KEH(nefE1,nf2)+NwfT*((Rzt1.*nx-Rxt1.*nz).*Nf);
        KEH(nefE2,nf2)=KEH(nefE2,nf2)+NwfT*((Rzt2.*nx-Rxt2.*nz).*Nf);
        KEH(nefE1,nf3)=KEH(nefE1,nf3)+NwfT*((Rxt1.*ny-Ryt1.*nx).*Nf);
        KEH(nefE2,nf3)=KEH(nefE2,nf3)+NwfT*((Rxt2.*ny-Ryt2.*nx).*Nf);
      end
      
      if nsd==2
        KEe(nefE1,nf1)=KEe(nefE1,nf1)-NwfT*((tauE*Rxt1).*Nf);
        KEe(nefE1,nf2)=KEe(nefE1,nf2)-NwfT*((tauE*Ryt1).*Nf);
      else
        KEe(nefE1,nf1)=KEe(nefE1,nf1)-NwfT*((tauE*Rxt1).*Nf);
        KEe(nefE2,nf1)=KEe(nefE2,nf1)-NwfT*((tauE*Rxt2).*Nf);
        KEe(nefE1,nf2)=KEe(nefE1,nf2)-NwfT*((tauE*Ryt1).*Nf);
        KEe(nefE2,nf2)=KEe(nefE2,nf2)-NwfT*((tauE*Ryt2).*Nf);
        KEe(nefE1,nf3)=KEe(nefE1,nf3)-NwfT*((tauE*Rzt1).*Nf);
        KEe(nefE2,nf3)=KEe(nefE2,nf3)-NwfT*((tauE*Rzt2).*Nf);
      end
      
      if nsd==2
        KEE(nefE1,nefE1)=NwfT*(((Rxt1.*Rxt1+Ryt1.*Ryt1)*tauE).*Nf);
      else
        KEE(nefE1,nefE1)=NwfT*(((Rxt1.*Rxt1+Ryt1.*Ryt1+Rzt1.*Rzt1)*tauE).*Nf);
        KEE(nefE2,nefE1)=NwfT*(((Rxt1.*Rxt2+Ryt1.*Ryt2+Rzt1.*Rzt2)*tauE).*Nf);
        KEE(nefE1,nefE2)=NwfT*(((Rxt1.*Rxt2+Ryt1.*Ryt2+Rzt1.*Rzt2)*tauE).*Nf);
        KEE(nefE2,nefE2)=NwfT*(((Rxt2.*Rxt2+Ryt2.*Ryt2+Rzt2.*Rzt2)*tauE).*Nf);
      end
    elseif isDirichlet_e
      if nsd==2
        KEE(nefE1,nefE1)=NwfT*((Rxt1.*Rxt1+Ryt1.*Ryt1).*Nf);
      else
        KEE(nefE1,nefE1)=NwfT*((Rxt1.*Rxt1+Ryt1.*Ryt1+Rzt1.*Rzt1).*Nf);
        KEE(nefE2,nefE1)=NwfT*((Rxt1.*Rxt2+Ryt1.*Ryt2+Rzt1.*Rzt2).*Nf);
        KEE(nefE1,nefE2)=NwfT*((Rxt1.*Rxt2+Ryt1.*Ryt2+Rzt1.*Rzt2).*Nf);
        KEE(nefE2,nefE2)=NwfT*((Rxt2.*Rxt2+Ryt2.*Ryt2+Rzt2.*Rzt2).*Nf);
      end
    elseif isAbsorbing
      if nsd==2
        KEH(nefE1,nf1)=KEH(nefE1,nf1)+NwfT*((Rxt1.*ny-Ryt1.*nx).*Nf);
