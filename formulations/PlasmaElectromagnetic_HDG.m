classdef PlasmaElectromagnetic_HDG < Formulation  
  ...
  methods
    ...
    function [Block,Elements]=buildBlock(~,iD,Block,Elements,~,Parameters,~,~,Time,RefElement,Sizes)
      ...
      Block(iD,iD).LhsGlobal=fsparse(Block(iD,iD).LhsRowIndices,Block(iD,iD).LhsColIndices,LhsCoef(:));
...
Je=reshape(SolutionLocal(:,qsd+nsd+qsd+(1:nsd)),[],1);
Ue=SolutionGlobal;
if isTimeDependent
  Holde=reshape(SolutionOld(:,1:qsd,:),[],BDFo);
  eolde=reshape(SolutionOld(:,qsd+(1:nsd),:),[],BDFo);
  Molde=reshape(SolutionOld(:,qsd+nsd+(1:qsd),:),[],BDFo);
  Jolde=reshape(SolutionOld(:,qsd+nsd+qsd+(1:nsd),:),[],BDFo);
end
